...
class FeedUIIntegrationTests: XCTestCase {
    ...
    func test_loadFeedCompletion_rendersErrorOnLoadFeedError() {
        let (sut, loader) = makeSUT()
    ...
    func test_feedImageView_cancelsImageLoadingWhenNotVisibleAnymore() {
        ...
        XCTAssertEqual(loader.cancelledImageURLs, [image0.url], "Expected one cancelled image URL request once first is not visible anymore")
        
        sut.simulateFeedImageViewNotVisible(at: 1)
        XCTAssertEqual(loader.cancelledImageURLs, [image0.url, image1.url], "Expected two cancelled image URL requests once second image also is not visible anymore")
    }
    ...
    func test_feedImageViewLoadingIndicator_isVisibleWhileLoadingImage() {
        ...
        XCTAssertEqual(view0?.isShowingImageLoadingIndicator, false, "Expected no loading indicator state change for first view once first image loading completes with error")
    ...
    func test_feedViewImage_rendersImageLoadedFormURL() {
        let (sut, loader) = makeSUT()
        ...
        XCTAssertEqual(view1?.renderedImage, .none, "Expected no image state change for second view once fist image loading completes successfully")
    ...
    func test_feedViewImageRetryButton_isVisibleOnImageURLLoadError() {
        let (sut, loader) = makeSUT()
        ...
        XCTAssertEqual(view1?.isShowingRetryAction, false, "Expected no retry action state change for second view once fist image loading completes successfully")
    ...
    func test_feedViewImageRetryButton_isVisibleOnInvalidImageData() {
        let (sut, loader) = makeSUT()
    ...
    func test_feedViewImageRetryAction_retriesImageLoad() {
        let image0 = makeImage(url: URL(string: "http://url-0.com")!)
        let image1 = makeImage(url: URL(string: "http://url-1.com")!)
        let (sut, loader) = makeSUT()
    ...
    func test_loadFeedCompletion_dispatchesFromBackgroudToMainThread() {
        let (sut, loader) = makeSUT()
    ...
    func test_loadImageDataCompletion_dispatchesFromBackgroudToMainThread() {
        let (sut, loader) = makeSUT()
